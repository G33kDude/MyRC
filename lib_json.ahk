...
; File encoding: UTF-8
;
; lib_json (http://www.autohotkey.com/forum/topic66070.html)
;    by lordkrandel
;
; Some tweaks by fincs
;
...
;JSON_save(obj, filename, spacing=35, spacer="    ", indent=1) {
; fincs: make output saner
JSON_save(obj, filename, spacing=1, spacer="   ", indent=1) {

	file         := FileOpen(filename, "w")
	jsonString   := JSON_to(obj, spacing, spacer, indent) "`n"
...
; fincs: make output saner
JSON_to(obj, spacing = 1, block = "   ", level = "1" ) {
...
JSON_init(){
	...
	JSON_rules[1] := Object( "key", "([QS]:[QSNOA])",                  "sub", "_", "func", "JSON_reduce_keyvalue" )
	JSON_rules[2] := Object( "key", "(\[(([QSNOA](,[QSNOA])*\])|\]))", "sub", "A", "func", "JSON_reduce_array"    )
...
JSON_shift(s, pos, symbols, ret){
	...
	for symbol,regexp in JSON_regexps {
		...
		RegexMatch(s, "P)(" . regexp . ")", match_, pos)
		if (match_pos1 == pos){
	...
	JSON_error("Error at pos:" pos "`n" substr(s,pos-4))
	;exit
}
...
JSON_reduce(symbols, ret){
	...
	while (rule_idx <= JSON_rules.maxIndex()) {
		...
		Regexmatch(symbols, "P)" . old_symbols . "$", match_)
